...
res <- resultstemplong[resultstemplong$predmethod=="rf" & resultstemplong$ytrend=="medium label shift",]

p <- ggplot(data=res, aes(x=type, y=mse, fill=type)) + theme_bw() +
  geom_boxplot() + facet_wrap(~ n + xtrend, ncol = 4, scales="free_y") +
  theme(axis.title.x=element_blank(), 
        axis.title.y=element_text(size=14), 
        axis.text.x = element_text(angle=45, hjust = 1, color="black", size=12), 
        axis.text.y = element_text(color="black", size=11), 
...
ggsave("./Simulations/concdrift/results/figures/mse_mediumlabeldrift_rf.pdf", width=11, height=9)


res <- resultstemplong[resultstemplong$predmethod=="lm" & resultstemplong$ytrend=="medium label shift",]
...
ggsave("./Simulations/concdrift/results/figures/mse_mediumlabeldrift_lm.pdf", width=11, height=9)
