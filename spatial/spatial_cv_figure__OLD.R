...
p1 <- ggplot() +
  geom_sf(data = bavaria, fill = "white", color = "black", linewidth = 1) +
...
p3 <- ggplot() +
  ...
  theme(legend.position = "none", axis.text=element_blank(),
        ...
        axis.title=element_blank()) +
  coord_sf() +
  ggtitle("k clustered groups with clustering based on the coordinates")
p3
...
p4 <- ggplot() +
  ...
  theme(legend.position = "none", axis.text=element_blank(),
        ...
        axis.title=element_blank()) +
  coord_sf() +
  ggtitle("Leave-one-out cross-validation with buffer")
...
p5 <- ggplot() +
  ...
  theme(legend.position = "none", axis.text=element_blank(),
        ...
        axis.title=element_blank()) +
  coord_sf() +
  ggtitle("Leave-one-disc-out cross-validation with buffer")
...
# Create the plot
p6 <- ggplot() +
  geom_sf(data = bavaria_districts, fill = "white", color = "black", linewidth = 1) +
...
ggsave("./spatial/results/figures/SpatialVis.pdf", plot=p, width=10*1.5, height=7*1.5)
...
set.seed(42) # for reproducibility
n_points <- 2000 # number of points to generate
grid_points <- st_sample(bavaria_districts, size = n_points, type = "random", crs = st_crs(bavaria_districts))
